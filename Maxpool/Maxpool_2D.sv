...
module Maxpool_2D #(
    ...
    always_comb begin
        ...
        case (state)
            IDLE: begin
                if (start)
                    next_state = COLLECT_INPUT;
            end

            COLLECT_INPUT: begin
                if (input_collection_done)
                    next_state = PROCESS;
            end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                PROCESS: begin
                    // Perform max pooling for current output position
                    logic [DATA_WIDTH-1:0] max_val;
                    logic signed [$clog2(IN_ROWS+3)-1:0] in_row;
                    logic signed [$clog2(IN_COLS+3)-1:0] in_col;
                    logic in_bounds;

                    max_val = (PADDING == 1) ? '0 : {DATA_WIDTH{1'b1}};
                    ...
                    for (int sr = 0; sr < SEG_ROWS; sr++) begin
                        for (int sc = 0; sc < SEG_COLS; sc++) begin
                            ...
                            if (in_bounds) begin
                                if (input_buffer[in_row][in_col] > max_val) begin
                                    max_val = input_buffer[in_row][in_col];
                                end
                            end else if (PADDING == 1) begin
                                // Padded values are zero, only update if current max is negative
                                if (max_val[DATA_WIDTH-1] == 1'b1) begin // max_val is negative
                                    max_val = '0;
                                end
                            end
                        end
