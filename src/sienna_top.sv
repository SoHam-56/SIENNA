...
module sienna_top #(
  ...
  logic [$clog2(FIFO_DEPTH+1)-1:0] fifo1_count_hw;
  logic [$clog2(FIFO_DEPTH+1)-1:0] fifo2_count_hw;
  logic [$clog2(FIFO_DEPTH+1)-1:0] fifo3_count_hw;
  logic fifo1_full_hw, fifo1_empty_hw;
  logic fifo2_full_hw, fifo2_empty_hw;
  logic fifo3_full_hw, fifo3_empty_hw;

  logic [$clog2(FIFO_DEPTH+1):0] fifo1_usage;
  logic [$clog2(FIFO_DEPTH+1):0] fifo2_usage;
  logic [$clog2(FIFO_DEPTH+1):0] fifo3_usage;

  logic [$clog2(FIFO_DEPTH+1):0] transfer_count;
  ...
  fwft #(
      ...
      .full_o (fifo1_full_hw),
      .rd_en_i(fifo1_rd_en),
      .data_o (fifo1_rd_data),
      .empty_o(fifo1_empty_hw),
      .count_o(fifo1_count_hw)
  );
  ...
  fwft #(
      ...
      .full_o (fifo2_full_hw),
      .rd_en_i(fifo2_rd_en),
      .data_o (fifo2_rd_data),
      .empty_o(fifo2_empty_hw),
      .count_o(fifo2_count_hw)
  );
  ...
  fwft #(
      ...
      .full_o (fifo3_full_hw),
      .rd_en_i(fifo3_rd_en),
      .data_o (fifo3_rd_data),
      .empty_o(fifo3_empty_hw),
      .count_o(fifo3_count_hw)
  );
  ...
  always_ff @(posedge clk_i or negedge rstn_i) begin
    if (!rstn_i) begin
      ...
      // Reset transfer count when we start a new GPNAE batch
      if (current_state == DRAIN_FIFO_TO_GPNAE) transfer_count <= 0;
      else if (current_state == COLLECT_GPNAE && gpnae_done_signal)
        transfer_count <= transfer_count + 1;

      case ({
        fifo1_wr_en, fifo1_rd_en
      })
        2'b10:   fifo1_usage <= fifo1_usage + 1;
        2'b01:   fifo1_usage <= fifo1_usage - 1;
        default: fifo1_usage <= fifo1_usage;
      endcase

      case ({
        fifo2_wr_en, fifo2_rd_en
      })
        2'b10:   fifo2_usage <= fifo2_usage + 1;
        2'b01:   fifo2_usage <= fifo2_usage - 1;
        default: fifo2_usage <= fifo2_usage;
      endcase

      case ({
        fifo3_wr_en, fifo3_rd_en
      })
        2'b10:   fifo3_usage <= fifo3_usage + 1;
        2'b01:   fifo3_usage <= fifo3_usage - 1;
        default: fifo3_usage <= fifo3_usage;
      endcase
    end
  end
  ...
  always_comb begin
    ...
    case (current_state)
      ...
      DROPOUT_PROCESSING: if (fifo3_usage == 0) next_state = PIPELINE_COMPLETE;
  ...
  always_comb begin
    fifo1_wr_en = 0;
    if (systolic_read_valid) fifo1_wr_en = 1;

    fifo3_wr_en = 0;
    if (maxpool_out_valid) fifo3_wr_en = 1;
    ...
    if (dropout_valid_out) final_output_reg_next = dropout_data_out;

    case (current_state)

      IDLE: systolic_read_addr_next = 0;

      SYSTOLIC_START_PULSE: systolic_start_next = 1;

      FEED_GPNAE_FIFO: begin

        if (fifo1_usage < FIFO_DEPTH) begin
      ...
      LATCH_GPNAE_COUNT: begin
        ...
        items_left_to_process_next = fifo1_usage;
        items_expected_back_next   = fifo1_usage;
      end

      DRAIN_FIFO_TO_GPNAE: begin
        if ((fifo1_usage > 0) && !gpnae_full && items_left_to_process > 0) begin
          fifo1_rd_en_next = 1;
          gpnae_wr_en_next = 1;
          if (gpnae_wr_en) items_left_to_process_next = items_left_to_process - 1;
          if (items_left_to_process == 1) gpnae_last_next = 1;
        end
      end
      ...
      COLLECT_GPNAE: begin
        ...
        if (gpnae_done_signal) begin
          fifo2_wr_en_next = 1;
          if (items_expected_back > 0) items_expected_back_next = items_expected_back - 1;
        end
      end

      PREP_MAXPOOL: begin
        items_left_to_process_next = transfer_count;
  ...
  assign intermediate_buffer_full_o = (fifo1_usage == FIFO_DEPTH);
  assign intermediate_buffer_empty_o = (fifo1_usage == 0);
